...
| A-only controlled training-duration run | training 완료·test recovery 대기 | epoch 24까지 수행, best validation e9=0.06152481; held-out output은 loop recovery로 재산출 |
| E24 checkpoint-only evaluator | 구현 완료·사용자 실행 대기 | saved checkpoint만 read-only로 평가해 누락된 test metric·candidate JSON 생성 |
