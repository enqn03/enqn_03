...
## 32. A-only 모델의 Signal 부재 한계 확인

이전까지의 모든 캘리브레이션과 검증은 A 모달리티(AfterSpreading)만을 이용한 인과적(Causal) Baseline을 확립하기 위한 과정이었다. 그러나 기하학적으로 완벽한 좌표계를 구축했음에도 불구하고, 실제 학습 과정에서 단일 모달리티가 가진 물리적 한계를 마주했다.

A-only 시계열은 레이저가 조사되기 전의 쇳가루 표면만을 관측한다. 이 표면은 이전 레이어의 열적 흔적(Thermal history)을 일부 내포할 수 있으나, 현재 레이어에서 결함이 발생할 것인지에 대한 직접적인 물리적 단서(Signal)가 현저히 부족하다. 즉, A-only 모델의 성능 한계는 모델의 용량(Capacity) 부족이 아니라, 입력 데이터 자체의 물리적 증거 부재에서 기인한다. 이로 인해 모델은 유의미한 결함을 특정하지 못하고 미탐(False Negative) 비율이 과도하게 높게 나타났다.

이를 해결하기 위해 실시간(Real-time) 모니터링의 정의를 '다음 층의 가루를 덮기 전 0.1초 내 인퍼런스 완료'로 현실화하고, 레이저 조사 후의 B 모달리티(Burned)를 적극 도입하기로 결정했다.

---

## 33. B-only 모델 도입과 과잉 탐지(False Positives) 현상

A 모달리티의 정보 부족을 보완하기 위해, 레이저 용융 순간을 포착한 B 모달리티 단독 학습(B-only)을 진행했다. B 이미지는 스패터(Spatter)와 용융풀(Melt pool) 형태 등 강렬한 시각적 특징을 담고 있어 결함 신호가 뚜렷할 것으로 가설을 세웠다.

실험 결과, 결함이 있는 구역을 찾아내는 재현율(Recall)은 상승했으나, 정상적인 레이저 궤적이나 단순 반사광마저 모두 결함으로 오인하는 과잉 탐지(False Positive) 문제가 심각하게 발생했다. 523번의 오탐이 발생하며 정밀도(Precision)는 7.8% 수준으로 하락했다. 이는 B-only 단일 모델이 스패터와 실제 구조적 결함을 구별하는 변별력이 부족함을 시사한다.

---

## 34. 단순 Concatenation의 실패

단일 모달리티의 한계를 극복하고자 A와 B 이미지를 채널 축으로 단순 병합(Concatenation)하는 초기 융합 모델을 시도했다. 그러나 B 이미지의 고휘도 노이즈(정상 반사광 및 스패터)가 A 이미지의 미세한 기하학적 텍스처를 압도(Overwrite)해버리는 현상이 발생했다. 그 결과, 단순 병합 모델은 B-only 모델과 동일하게 과도한 오탐을 반복하는 양상을 보였다.

---

## 35. Gated CBAM Fusion 아키텍처 설계

단순 병합의 한계를 해결하기 위해, A의 기하학적 맥락 정보로 B의 노이즈를 필터링하는 Gated Fusion 아키텍처를 고안했다.

1. **Gated Activation:** A 이미지의 특징 맵(Feature map)을 기반으로 Sigmoid 기반 게이트를 생성하여, 구조적으로 안정적인 지형에서는 B의 강렬한 신호라도 그 영향력을 0으로 억제(Gate)하도록 설계했다.
2. **CBAM (Convolutional Block Attention Module):** 채널 어텐션과 공간 어텐션을 순차적으로 적용하여, 모델이 불필요한 전체 배경 노이즈를 무시하고 붉은 레이저 코어 주변의 실제 이상 징후에만 선택적으로 집중하도록 유도했다.

이러한 모달리티 융합은 단순한 정보 더하기가 아닌, 시계열적 교차 검증 역할을 수행한다.

---

## 36. 극불균형 데이터를 위한 Masked BCE Loss 최적화

데이터셋 내 결함 픽셀의 비율은 1% 미만으로 극도의 클래스 불균형(Class Imbalance) 상태다. 초기에는 사후 XCT 점수(0~1)를 직접 회귀(SmoothL1)하도록 유도했으나, 모델이 모든 픽셀을 0(정상)에 가깝게 예측하여 Loss를 최소화하려는 자명한(Trivial) 해로 수렴하는 문제가 발견되었다.

이를 해결하기 위해, 모델이 연속적인 점수를 예측하기 전에 픽셀의 결함 여부 자체를 이진 분류(Classification)하도록 강제하는 **Masked BCE (Binary Cross Entropy) Loss**를 주 손실 함수에 결합했다. 또한 허공 등 데이터가 없는 영역을 0으로 오학습하지 않도록 확실한 부품 영역(Support region)에서만 오차를 계산(Masking)함으로써 학습 안정성을 확보했다.

---

## 37. 하이퍼파라미터 변인 통제 및 모델 최적화 실험

퓨전 아키텍처의 성능 극대화 및 과적합 방지를 위해 철저한 하이퍼파라미터 실험을 진행했다.

- **채널 용량(Channel Capacity) 실험:** 베이스 채널 크기를 8, 16, 32로 비교 실험했다. C32는 과적합 위험이 높고 VRAM 점유율이 컸으며, **C16**이 제한된 자원 하에서 실시간 추론을 보장하면서도 A/B 특징 결합에 최적화된 스윗스팟임을 확인했다.
- **다중 손실 가중치(Loss Weight) 스윕:** `SmoothL1(회귀)`과 `BCE Loss(분류)`를 결합할 때 두 손실의 반영 비율을 변경해가며 F1-Score가 정점을 찍는 구간을 확인했다.
- **강력한 정규화(Regularization) 도입:** 게이트와 CBAM 모듈 추가로 인한 파라미터 증가에 대응하기 위해, 디코더에 **Spatial Dropout(30%)**을 추가하고 AdamW 옵티마이저의 **Weight Decay를 0.01**로 설정하여 과도한 노이즈 암기 현상을 억제했다.
- **학습 주기(Epoch) 검증:** Epoch 8과 24를 비교하여, 모델이 제대로 예측을 못 하는 원인이 단순히 덜 학습(Under-training)되어서인지 아키텍처의 한계인지를 통제된 변인 속에서 하나씩 소거해 나갔다.

---

## 38. Ablation Study: 오탐 억제와 최종 성능

객체 단위(2mm 허용 오차 반경) 최종 성능 평가를 통해 Ablation Study를 수행했다.

- **B-only 모델:** 오탐 523개, Precision 7.8%
- **Gated CBAM Fusion 모델:** 오탐 111개, Precision 대폭 상승

A의 기하학적 정보가 B의 시각적 노이즈를 성공적으로 필터링(Gate)하여, 실제 결함을 놓치지 않으면서도 오탐을 약 5배 축소시켰고, 결과적으로 F1-Score를 단일 모델 대비 약 2배 향상하는 최적의 밸런스를 달성했다.

---

## 39. 실시간 3D 조기 경보 파이프라인(Live Inference) 구축

오프라인 기반 사후 XCT 검사의 한계를 넘어, 학습된 퓨전 모델을 기반으로 한 실시간 층별 모니터링(Layer-by-Layer Monitoring) 파이프라인을 구축했다. 
장비가 프린팅을 진행할 때 입력되는 A/B 카메라 이미지를 즉시 인퍼런스하여, 물리적 절대 좌표계(Machine X, Y mm) 기준의 `(x, y, z, score)` 결함 후보군을 실시간으로 터미널에 출력하고 CSV로 저장한다.

---

## 40. 결론 및 한계점 (Next Steps)

최종적으로 도출된 `live_inference_stream` 모듈을 통해 공정 엔지니어가 실시간으로 결함 의심 구역의 3D 분포도를 모니터링할 수 있는 조기 경보 시스템(Early Warning System)의 Baseline이 완성되었다. 

다만 현재 시스템은 확정된 결함 판정 및 자동 공정 제어 신호로 사용되기에는 다음의 과제들이 남아있다.
1. **Weak Labels 매칭의 기하학적 한계:** 1픽셀 단위의 완벽한 2D-3D 매칭은 물리적으로 불가능하므로, 향후 주변 맥락을 유연하게 학습하는 Vision Transformer나 Self-Supervised Learning의 도입이 필요하다.
2. **실시간 추론 속도 최적화:** 실 공정에 탑재하기 위해 CBAM 연산의 경량화(ONNX 변환, TensorRT, Pruning 등) 엔지니어링 작업이 요구된다.
3. **결함 다중 분류 확장:** 현재의 '이상 확률' 기반 1차 후보군 제안을 넘어, 기공, 크랙 등 세부 결함 원인을 진단하는 차세대 분류 시스템으로 고도화해야 한다.
