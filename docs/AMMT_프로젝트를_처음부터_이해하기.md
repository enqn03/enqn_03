...
## 37. 변인 통제 및 모델 하이퍼파라미터 최적화

퓨전 아키텍처의 성능 극대화 및 과적합 방지를 위해 철저한 하이퍼파라미터 실험을 진행했다.

- **Channel Capacity:** 베이스 채널 크기를 8, 16, 32로 비교 실험했다. C32는 과적합 위험이 높고 VRAM 점유율이 컸으며, **C16**이 제한된 자원 하에서 실시간 추론을 보장하면서도 A/B 특징 결합에 최적화된 스윗스팟임을 확인했다.
- **Regularization:** 게이트와 CBAM 모듈 추가로 인한 파라미터 증가에 대응하기 위해, 디코더에 **Spatial Dropout(30%)**을 추가하고 AdamW 옵티마이저의 **Weight Decay를 0.01**로 설정하여 과도한 노이즈 암기 현상을 억제했다.
- **다중 시드(Multi-Seed) 통제:** 42, 100, 2026 등 무작위 시드를 달리하여 재학습을 진행한 결과, F1-Score의 분산이 거의 없이 안정적인 성능을 상회함을 확인하여, Gated CBAM 구조의 필터링 능력이 통계적으로 유의미함을 입증했다.
