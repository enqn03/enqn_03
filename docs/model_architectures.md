...
전체 파이프라인은 시점에 따라 **조기 경보**와 **사후 종합 평가**로 나뉩니다.
...
* **목적:** 레이저를 쏘기 전, 가루가 덮인 상태만 보고 이상을 예측하는 **조기 경보** 모델입니다.
* **구조적 특징:** 가루 표면은 극적인 변화가 없기 때문에, 이전 레이어와 현재 레이어의 미세한 **차이**에만 집중하도록 시계열 차분 인코더를 사용합니다.
...
* **목적:** 레이저를 쏜 직후의 쇳물과 스패터 열화상을 분석하는 모델입니다.
* **구조적 특징:** 결함에 대한 가장 강력하고 직접적인 신호를 담고 있으며, 단일 모델 중 가장 높은 정확도를 기록한 베이스라인입니다. A-only와 동일한 구조지만 입력 데이터만 B-stage를 사용합니다.
...
* **목적:** 가루 상태와 쇳물 상태의 정보를 모두 활용하여 최고의 정확도를 끌어내기 위한 최종 병기입니다.
* **구조적 특징:** 
  1. A와 B를 독립적인 인코더로 각각 처리합니다.
  2. 단순 병합 시 발생하는 **노이즈 간섭**을 막기 위해, **Gated CNN** 모듈이 A와 B의 특징을 분석하여 어떤 채널과 공간의 정보를 살리고 죽일지 스스로 밸브를 조절합니다.
  3. 강력한 표현력으로 인한 **과적합**을 막기 위해, 디코더에 **Spatial Dropout (30%)**을 적용하여 정보의 일부를 랜덤하게 차단합니다.
...
graph LR
    ...
    A_ENC --> GATED{Gated CBAM Fusion<br>채널 및 공간 어텐션 기반 노이즈 밸브 차단}
    B_ENC --> GATED
...
단순히 파라미터 값을 나열하는 데 그치지 않고, 실제 공정 데이터가 가진 한계(노이즈, 불균형, 오차)를 극복하기 위해 아래와 같이 하이퍼파라미터를 통제했습니다.

### 모델별 하이퍼파라미터 설정표

| 파라미터 | A-only (조기경보) | B-only (단일최강) | A+B Fusion (v6 실무용) | A+B Fusion (v7 튜닝용) |
| :--- | :---: | :---: | :---: | :---: |
...
- **현상:** 결함은 한 층의 실수뿐 아니라 여러 층에 걸친 열 누적으로 서서히 발생하기도 합니다.
- **해결:** 과거 4개 레이어 이미지를 묶어 모델이 열적 및 구조적 변화의 **추세**를 인지하도록 설계했습니다.

#### 2. 정규화 및 과적합 방지 (Spatial Dropout & Weight Decay)
- **현상:** 결함 데이터가 극도로 희귀하여, 파라미터가 거대한 Fusion 모델은 특정 결함 픽셀 위치를 그대로 **암기**해버리는 과적합이 쉽게 발생합니다.
- **해결:** 
...
> 위 표를 보면 30-Epoch(v7) 튜닝 모델이 14 에포크 부근에서 검증 손실 0.09까지 떨어집니다. 하지만 직전과 직후에 0.48~0.57로 극심하게 요동을 칩니다.  
> 
> 가장 손실이 낮았던 14 에포크 모델을 미지의 테스트 셋에 돌려본 결과, **8-Epoch 모델은 테스트 손실 3.4로 선방한 반면 30-Epoch 모델은 6.4로 성능이 완전히 붕괴**되었습니다.  
> 즉, 0.09는 진짜 실력이 아닌 우연히 검증 셋 노이즈에 맞아떨어진 **거짓된 과적합**이었으며, 미지의 데이터에 대해 가장 튼튼한 일반화 성능을 보인 8-Epoch 모델을 실무용으로 채택했습니다.

#### 3. 가우시안 타겟 릴렉세이션 (Gaussian Target Sigma=2)
- **현상:** 사후 정답지를 2D 카메라 좌표계로 투영할 때 1~2 픽셀의 기계적 오차가 필연적으로 발생합니다.
- **해결:** 정답 픽셀에 반경 2-Pixel의 가우시안 블러를 적용하여 기계적 오차를 유연하게 수용했습니다. 칼채점을 하지 않음으로써 실제 현장 결함 검출력이 폭발적으로 상승했습니다.

#### 4. 결함 임계값 (Binarization Threshold=0.85)
- **현상:** 애매한 회색 지대 점수를 정답지로 주면 모델의 결정 경계가 흐려집니다.
- **해결:** 스캔 점수 상위 15% 이상만 결함으로 취급하는 극단값 타겟팅을 통해, 스패터 폭발이나 파우더 뭉침 같은 **확실한 시각적 특징만 학습**하도록 유도했습니다.
