...
2. **정규화 및 과적합 방지 (Spatial Dropout=0.3, Weight Decay=0.01 → 0.05 Tuning)**
   ...
   - **🔍 분석 심화 (왜 최저점 Loss를 찍은 30-Epoch가 아닌 8-Epoch 모델을 최종 선택했는가?):** 
     위 차트를 보면 30-Epoch(v7) 튜닝 모델이 14 에포크 부근에서 0.09까지 Validation Loss가 일시적으로 낮아지지만, 그 직전과 직후에 0.48~0.57로 극심하게 요동(Oscillation)치는 것을 볼 수 있습니다. 가장 Loss가 낮았던 14 에포크의 가중치를 추출하여 완전히 미지의 데이터인 **Test Set**에 검증해 본 결과, 8-Epoch(v6) 모델의 Test Loss는 3.4로 방어된 반면, 30-Epoch(v7) 모델의 Test Loss는 6.4로 두 배 가까이 치솟으며 성능이 완전히 붕괴되었습니다. 즉, 0.09라는 수치는 모델의 진짜 실력이 아니라 거대한 가중치가 튀다가 우연히 검증 데이터의 노이즈에 맞아떨어진 **'거짓된 과적합(Lucky Cherry-picking)'**이었음이 증명되었습니다. 따라서 우리는 일시적 숫자에 현혹되지 않고 미지의 데이터에 대해 가장 일관된 방어력(Generalization)을 보여준 8-Epoch 모델을 실무 투입용으로 최종 채택했습니다.
