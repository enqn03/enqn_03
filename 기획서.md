...
3. **정규화 디코더 (Regularized Decoder):** 융합된 정보를 바탕으로 최종 이상 후보 점수를 공간 맵(Map) 형태로 출력합니다. 게이트 모듈 추가로 인한 심각한 과적합(Overfitting)을 방지하기 위해 **Spatial Dropout(30%)**과 **강력한 AdamW 가중치 감쇠(Weight Decay=0.01)**가 적용되어 있습니다.
