...
def extract_patches() -> None:
    ...
    for i in range(len(dataset)):
        ...
        # We only care about supported pixels
    ...
    def extract_patch(image_tensor: torch.Tensor, y: int, x: int) -> np.ndarray:
        # image_tensor is [K, C, H, W]. We want endpoint K=-1, LED3 which is C=2
        # C=0: LED1, C=1: LED2, C=2: LED3
        # Valid masks are C=3,4,5. We just take intensity.
        # Ensure padding if patch goes out of bounds
