...
def main():
    ...
    # Forward Hook을 사용해 Spatial Attention 맵 가로채기
    spatial_attention_maps = []
    def spatial_hook(module, inp, out):
        # out은 sigmoid를 거치기 전의 Logit 값이므로 sigmoid 적용 후 저장
    ...
    with torch.no_grad():
        ...
        probs = probs * mask  # Support mask 적용
        
    prob_map = probs[0, 0].cpu().numpy()
    
    # hist_a shape: [1, K, C, H, W] (e.g. [1, 2, 6, 256, 256])
    # K=마지막 인덱스(-1)의 프레임에서, C=0 (Intensity 채널) 추출
    img_a = hist_a[0, -1, 0].cpu().numpy()
    img_b = hist_b[0, -1, 0].cpu().numpy()
    
    # 어텐션 맵 해상도 복원 (원본 해상도 256x256으로 Upsample)
