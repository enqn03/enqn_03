...
swaps the input stage from A (AfterSpreading) to B (Burned). It preserves
the causal split, K=4 six-channel inputs, ROI, normalization, sparse XCT-derived
continuous weak target, masked loss, and raw-camera decoder.
"""
...
class BOnlyCausalTemporalDifferenceCandidateNet(nn.Module):
    """B-only model with explicit causal endpoint-minus-prior feature fusion.

    The input remains [B,K=4,C=6,H,W] from the B-stage. Each frame is encoded independently.
    The three prior encoded frames are averaged, subtracted from endpoint
    encoding, transformed, and fused with endpoint encoding.
    """
...
def make_b_dataset(args: argparse.Namespace, split: str) -> AMMTWeakTargetDataset:
    return AMMTWeakTargetDataset(
        stage="B",
        tiff_path=args.tiff_b,
...
def validate_config(config: dict[str, Any]) -> None:
    ...
    if data["stage"] != "B" or int(data["input_channels"]) != 6:
        raise ValueError("Temporal-difference training requires B-only six-channel input.")
    ...
    if model["name"] != "b_only_causal_temporal_difference_candidate_net":
...
def dry_run(model: nn.Module, dataset: Any, loss_fn: SupportMaskedBCELoss, device: torch.device, sample_index: int) -> None:
    ...
    print(
        json.dumps(
            {
                "audit_type": "B-only temporal-difference controlled experiment dry run",
    ...
    print("Temporal-difference dry run complete.")
...
def main() -> None:
    ...
    loss_fn = SupportMaskedBCELoss(binary_defect_threshold=0.85, pos_weight=100.0)
    train_dataset = make_b_dataset(args, split="train")
    if args.dry_run:
    ...
    validation_dataset = make_b_dataset(args, split="validation")
    ...
    test_dataset = make_b_dataset(args, split="test")
    ...
    for epoch in range(1, epochs + 1):
        ...
        # Ensure we skip None losses during comparison
    ...
    write_json(
        ...
        {
            "purpose": "B-only Temporal-difference training history",
    ...
    write_json(
        ...
        {
            "purpose": "B-only causal test metric on sparse XCT-supported pixels",
    ...
    write_json(
        ...
        {
            "purpose": "B-only compact raw-camera coordinate candidates",
            "stage": "B",
            "response_direction": "unresolved",
        ...
        json.dumps(
            {
                ...
                "architecture": "b_only_temporal_difference",
                "device": str(device),
                "best_epoch": best_epoch,
                "best_validation_supported_pixel_mean_smooth_l1": best_validation_loss,
                "test": asdict(test_metrics),
    ...
    print("B-only Temporal-difference controlled experiment complete.")
