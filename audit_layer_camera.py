...
def map_frame(data: np.memmap, info: TiffInfo, z: int, led: int) -> np.ndarray:
    ...
    if "Z" in info.axes and "T" in info.axes:
        ...
        # ImageJ XYCZT flatten order: C -> Z -> T. C=1 for AMMT.
...
def main() -> None:
    ...
    # These are the only files this script may create or overwrite.
    ...
    # Preflight all output paths before reading frames, so the script stops
    # without partial results if an existing file needs user review.
    ...
    # Evenly sample requested audit range for contact sheet.
