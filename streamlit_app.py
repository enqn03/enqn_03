...
with tab_conclusion:
    ...
    st.subheader("🎯 기존 목표")
    st.markdown("""
    - 사후 XCT 파괴/비파괴 검사에 절대적으로 의존하던 기존 금속 3D 프린팅(AMMT) 결함 검사 패러다임을 **공정 중(In-situ) 모니터링**으로 전환
    - 파우더 도포 후(A)와 레이저 조사 후(B) 이미지를 융합하여 미세 결함 발생 위치를 **실시간(Layer 단위)으로 예측 및 시각화**
    """)
    
    st.divider()
    st.subheader("✅ 현재 결과")
    st.markdown("""
    - **A+B Gated CBAM Fusion 아키텍처 개발:** 단일 이미지(A or B) 분석의 한계를 완벽히 극복하고, 시계열적 교차 검증을 통해 오탐(False Alarms)을 523회에서 111회로 획기적으로 축소했습니다.
    - **평가 기준의 현실화 (Pixel vs Blob):** 산업 현장의 물리적 좌표 오차를 고려하여 2mm 객체(Blob) 단위 평가 지표를 성공적으로 도입하였고, 그 결과 Recall이 10배 폭증(4.5% → 43.5%)하며 숨겨진 모델 성능을 증명해냈습니다.
    - **실시간 모니터링 파이프라인 완성:** 2D 픽셀 카메라 좌표를 3D 물리(Machine) 좌표계로 정밀하게 맵핑(Calibration)하여, 엔지니어가 실시간으로 결함 의심 구역을 3D 시뮬레이션으로 모니터링할 수 있는 대시보드 시스템을 구현했습니다.
    """)
    
    st.divider()
    st.subheader("🚀 한계점 및 향후 발전 방향")
    st.markdown("""
    - **Weak Target 학습의 기하학적 매칭 한계:** XCT 정답지와 2D 레이어 이미지 간의 1픽셀 단위 완벽한 기하학적 매칭은 기계적 한계로 불가능하며, 여전히 칼채점(픽셀 단위) 시 성능이 저평가되는 문제가 남아있습니다.
      > 💡 **발전 방향:** 차후 Vision Transformer(ViT)나 Self-Supervised Learning(자기 지도 학습) 기법을 도입하여, 약한 정답지(Weak Labels) 환경에서도 주변 픽셀의 맥락(Context)을 더 유연하게 이해하는 모델로 고도화할 수 있습니다.
    - **실시간 추론 속도 (Latency) 최적화 이슈:** 현재 모델은 정확도 극대화를 위해 무거운 연산(CBAM 등)을 수행하고 있으나, 실제 공정(수천 레이어)에 실시간 탑재하기 위해서는 경량화가 필수적입니다.
      > 💡 **발전 방향:** ONNX 모델 변환, TensorRT 적용 또는 모델 Pruning(가지치기) 및 Quantization(양자화)을 통해 딥러닝 추론 속도를 밀리초(ms) 단위로 깎아내는 엔지니어링 최적화 작업이 필요합니다.
    - **다양한 결함 원인 분석 확장:** 현재는 결함 '확률(Score)'을 중심으로 의심 위치를 좁히는 1차 모니터링에 주력하고 있습니다.
      > 💡 **발전 방향:** 결함의 '유형(크랙, 기공, 층간 분리 등)'을 세부적으로 다중 분류(Multi-class Classification)하는 기능까지 확장하여, 엔지니어에게 구체적인 원인 분석 리포트까지 자동 제공하는 차세대 시스템으로 발전할 수 있습니다.
    """)
