...
with tab1:
    ...
    **LPBF(Laser Powder Bed Fusion)** 기반 금속 3D 프린팅 공정은 한 번에 제품을 만드는 것이 아니라, 수천 개의 레이어를 한 층씩 쌓아 올려 완성합니다.
    ...
    본 프로젝트는 이러한 한계를 극복하기 위해, 프린터 내부에 설치된 **공정 카메라(Layer-Camera) 데이터**를 실시간으로 분석하여 **내부 XCT 결함을 공정 중에 조기 예측(Early Warning)**하는 인공지능 기반 모니터링 시스템을 개발하는 것을 목표로 합니다.
    """)
    
    st.header("2. 핵심 프로젝트 개념 및 출력물의 의미")
    st.markdown("""
    - **제조 환경 2D 이미지의 3D 정답화:** 의료용 3D 볼륨 데이터가 아닌, 프린팅 중 촬영되는 단면 2D 시퀀스 이미지를 모델의 입력값으로 사용합니다. 완성품의 사후 XCT 3D 스캔 결과는 캘리브레이션 맵핑을 거쳐 모델 학습을 위한 **약한 정답지(Weak Supervision Target)**로 활용됩니다.
    - **연속적인 품질 점수(Quality Candidate) 제안:** 
      본 모델의 최종 출력은 단순히 픽셀 단위로 "결함이다/아니다"를 이분법적으로 확정 짓는 것이 아닙니다. 대신, 카메라 시퀀스를 분석하여 엔지니어가 우선적으로 검토해야 할 이상 의심 위치를 **`[x픽셀, y픽셀, 레이어층, 확률 점수(Score)]`** 형태로 연속성 있게 제안합니다.
    - **시계열 인과적 학습 (Causal Sequence):**
    ...
    st.markdown("""
    초기에는 A(파우더 도포 후)와 B(레이저 용융 후) 이미지를 하나의 신경망으로 통합 처리하려 했으나, 두 공정 이미지 간의 극심한 밝기 편차(Photometric Difference)와 예측 불가한 스패터(Spatter) 노이즈 간섭으로 인해 완전히 독립적인 듀얼-스트림 처리가 필수적임을 확인했습니다.
    
    - **독립적 특징 추출:** 파우더 도포 후(A)와 레이저 조사 후(B) 이미지를 섞지 않고, 각각 독립된 인코더(Encoder)를 통해 고유한 시각적 특징을 추출합니다.
    - **Temporal Difference 적용:** 단순히 현재 시점의 이미지만 보는 것을 넘어, 직전 과거 프레임들의 평균과 현재 프레임의 차이(Difference)를 명시적으로 계산하여 공정 상의 '변화량'에 모델이 집중하도록 유도했습니다.
    - **게이트 융합 (Gated CBAM Attention):** A와 B에서 추출된 특징을 결합할 때, CBAM(Convolutional Block Attention Module)이 채널 및 공간적 어텐션을 동적으로 계산합니다. 이를 통해 B 이미지의 시끄러운 노이즈는 억제하고, 결함 탐지에 유효한 시그널만 선별적으로 융합(Fusion)하여 오탐을 획기적으로 줄였습니다.
    """)
...
with tab_process:
    ...
    st.markdown("""
    - **왜 B-A를 검증했는가? (목적):** 정적인 단일 이미지(A 또는 B)만으로는 표면의 단순한 얼룩과 실제 구조적 결함을 구분하기 어렵습니다. 따라서 파우더 도포(A) 후 레이저 조사(B)가 이루어지는 과정에서 발생하는 **'시각적 차이(B-A)'**가 결함을 짚어내는 강력한 신호(Temporal Difference)가 될 수 있는지 검증하고자 했습니다.
    - **검증 과정:** `audit_ab_pairs` 과정을 통해 수천 장의 A/B 프레임 페어링이 기하학적으로 완벽히 정렬(Alignment)되는지 확인하고, 두 이미지의 픽셀 차이(Absolute A/B difference)를 추출하는 읽기 전용 검증(Read-only Audit)을 수행했습니다. 
    """)
    ...
    st.markdown("""
    - **무엇을 얻었는가? (결과):** 검증 결과, 장비의 고정된 구조물이나 조명 그림자 같은 불필요한 배경(Static Background)은 완벽하게 제거되고, 오직 레이저로 인해 새롭게 용융된 부분과 비정상적인 스패터(Spatter) 발생 구역만 선명하게 도드라지는 것을 시각적/정량적으로 입증해 냈습니다.
    - **다음으로 무엇을 했는가? (Next Step):** 단순한 B-A 차이 이미지조차 강력한 '이상 후보 맵(Candidate Map)'으로 기능함을 확인한 후, 이 데이터를 인공지능 모델에 깨끗하게 집어넣기 위해 **최적의 분석 영역(ROI)을 설정하고 조명 편차를 잡는 정규화(Normalization)** 작업으로 넘어갔습니다. 이후 이 통찰은 최종 A+B 퓨전 모델 아키텍처를 설계하는 핵심 근거가 되었습니다.
    """)
    ...
    - **왜 상위/하위 샘플 라벨링이 필요했는가?:** 원본 XCT 데이터에는 명확한 **정상(Normal) / 결함(Defect) 이진 라벨(Label)이 존재하지 않았습니다.** 오직 연속적인 물리적 밀도/결함 점수(Score)만이 존재했습니다. 따라서, 어설픈 중간 점수들을 배제하고 확실한 차이를 학습시키기 위해 XCT 점수 기준 최상위(하위) 샘플 패치들을 추출하여 분석(Target Semantics)하고, 이를 토대로 모델이 명확하게 결함의 특징을 학습할 수 있도록 라벨링 및 타겟을 세팅했습니다.
    - **라벨링(Labeling)은 어떻게 진행되었는가?:**
      1. **Robust Scaling (정규화):** 연속적인 XCT 점수의 상/하위 1% 극단값(Outlier)을 잘라내고(p01~p99), 전체 점수 스펙트럼을 0에서 1 사이로 매끄럽게 정규화(Scaling)했습니다.
      2. **임계값(Threshold) 기반 이진화:** 정규화된 점수가 **0.85 이상(상위 15%)인 구역만을 치명적인 결함(Defect, Label 1)**으로 확정 짓고, 나머지를 정상(Label 0)으로 분류했습니다.
      3. **2D 가우시안 래스터화(Gaussian Rasterization):** 3D 공간상의 결함 포인트들을 2D 평면 이미지 좌표로 투영할 때, 픽셀 간의 끊김을 방지하고자 2-Pixel 반경의 가우시안 블러(Gaussian Weighted Average)를 적용하여 부드러운 히트맵(Heatmap) 형태의 정답지를 생성했습니다.
      4. **마스킹(Masking):** 실제 부품이 얹혀지지 않은 허공(Outside Support) 영역은 학습에 방해되지 않도록 `Unknown` 처리하여 손실 함수(Loss) 계산에서 완전히 제외시켰습니다.
    """)
...
with tab_conclusion:
    ...
    - **평가 기준의 현실화 (Pixel vs Blob):** 산업 현장의 물리적 좌표 오차를 고려하여 2mm 객체 단위 평가 지표를 성공적으로 도입하였고, 그 결과 Recall이 10배 폭증(4.5% → 43.5%)하며 숨겨진 모델 성능을 증명해냈습니다.
    ...
    - **다양한 결함 원인 분석 확장:** 현재는 결함 '확률(Score)'을 중심으로 의심 위치를 좁히는 1차 모니터링에 주력하고 있습니다.
