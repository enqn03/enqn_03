...
with tab1:
    ...
    col3.metric("F1-Score", "25.38%", "결함 비율 0.8% 극불균형 환경")
    st.divider()

    st.header("1. 프로젝트 배경 및 문제 정의")
    st.markdown("""
    **LPBF(Laser Powder Bed Fusion)** 기반 금속 3D 프린팅 공정은 한 번에 제품을 만드는 것이 아니라, 수천 개의 레이어를 한 층씩 쌓아 올려 완성합니다.
    기존 제조 환경에서는 제품이 완전히 완성된 후 **사후 XCT(X-ray Computed Tomography) 스캔을** 통해서만 내부 결함을 파악할 수 있었습니다. 만약 프린팅 초반부에 결함이 발생했더라도 이를 모른 채 며칠간 공정을 지속하게 되어 막대한 **시간과 원자재 손실이** 발생하게 됩니다.
    본 프로젝트는 이러한 한계를 극복하기 위해, 프린터 내부에 설치된 **공정 카메라 데이터를** 실시간으로 분석하여 **내부 XCT 결함을 공정 중에 조기 예측하는** 인공지능 기반 모니터링 시스템을 개발하는 것을 목표로 합니다.
    """)
...
with tab_process:
    ...
    st.divider()
    st.subheader("분석 대상 데이터셋 스펙 및 제약 조건")
    st.markdown("본 프로젝트에서 다룬 LPBF 공정 카메라 데이터셋의 물리적 제약 조건 및 타겟 불균형 수준은 다음과 같습니다.")
    c1, c2, c3, c4 = st.columns(4)
    c1.metric("분석 대상 3D 시퀀스", "250+ Layers", "전체 Layer")
    c2.metric("단일 프레임 해상도", "2000 x 2000 px", "대용량 영상")
    c3.metric("카메라 센서 포화도", "최대 65535", "극심한 조명 간섭")
    c4.metric("실제 타겟(결함) 비율", "0.8% 미만", "극불균형 데이터")
    st.markdown("> **결론:** 위와 같은 가혹한 산업 현장의 광학적 제약과 극불균형 타겟 조건을 극복하기 위해, ROI 최적화, 6채널 입력, 정규화, 가우시안 마스킹, Gated CBAM 어텐션 기반의 정교한 딥러닝 파이프라인 설계가 요구되었습니다.")
    st.divider()
...
with tab_analysis:
    ...
    st.markdown("""
    우리 프로젝트는 단순히 모델 결과를 나열하는 것을 넘어, **독립 변인을 철저히 통제한 실험을** 통해 Recall과 F1-Score가 어떻게, 그리고 왜 향상되었는지 합리적으로 증명합니다.
    """)
    st.divider()
    ...
    with col2:
        ...
        **분석 결과 및 공학적 원인:**
        ...
        - **A+B Fusion (우리 모델):** 
          > **왜 B-only보다 성능(F1)이 우수한가?** 두 이미지를 융합하면 모델이 **시계열적 교차 검증**을 수행할 수 있습니다. 즉, 파우더 도포(A) 상태에서 미세한 이상이 있었던 위치에 불규칙한 용융(B)이 발생했을 때만 이를 **확실한 결함**으로 판단하게 됩니다. 이로 인해 불필요한 오답이 획기적으로 줄어들어, **F1-Score가 단일 모델 대비 약 2배(25.4%)로 수직 상승하는** 최적의 밸런스를 달성했습니다.
        """)
    st.divider()
    ...
    with col_text:
        ...
        **분석 결과:**
        ...
        - **결론:** 통제 변인(모델)은 동일함에도 불구하고 평가 기준 하나만 현실적으로 수정했을 뿐인데, **Recall이 4.5%에서 43.5%로 무려 10배 폭증함**. 즉, 우리 모델은 이미 결함을 정확히 찾고 있었음을 증명.
        """)
    st.divider()
    ...
    with col2:
        ...
        또한 XCT 데이터가 제공되지 않는 구멍이나 빈 공간을 **정상이라고** 잘못 학습하는 것을 막기 위해, 데이터가 확실히 존재하는 부품 내부 영역에서만 오차를 계산하도록 손실 함수를 재설계하여 학습 안정성을 극대화했습니다.
        """)
    st.divider()
    ...
    if os.path.exists(csv_path):
        ...
        def run_live_simulator():
            ...
            if show_cumulative:
                ...
                list_title = f"**누적 {len(df_filtered)}개의** 결함 의심 구역 발견 ({min_layer}층 ~ {current_z}층)"
            else:
                df_filtered = df[df['layer_z'] == current_z]
                list_title = f"**해당 층 {len(df_filtered)}개의** 결함 의심 구역 발견 (현재 {current_z}층)"
...
with tab_conclusion:
    ...
    st.subheader("기존 목표")
    st.markdown("""
    - 사후 XCT 파괴/비파괴 검사에 절대적으로 의존하던 기존 금속 3D 프린팅 결함 검사 패러다임을 **공정 중 모니터링으로** 전환
    - **초기 접근의 한계와 목표 전환:** 기존에는 파우더 도포 후(A) 과정의 이미지만으로 불량을 검출해내려 했으나, A 모델 데이터의 한계(레이저 조사 전 상태)로 인해 A 단독으로는 결함을 짚어내지 못하는 문제가 발견되었습니다.
    - 이에 따라 파우더 도포 후(A)와 레이저 조사 후(B) 이미지를 융합하여 미세 결함 발생 위치를 **실시간(Layer 단위)으로 예측 및 시각화하는** 방향으로 프로젝트 목표를 전환하고 고도화했습니다.
    """)
    st.divider()
    st.subheader("현재 결과")
    st.markdown("""
