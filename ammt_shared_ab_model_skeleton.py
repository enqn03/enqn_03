...
class FrameEncoder(nn.Module):
    ...
    def forward(self, frame: Tensor) -> Tuple[Tensor, Tensor, Tensor]:
        skip1 = self.level1(frame)  # [B, base, H, W]
        skip2 = self.level2(skip1)  # [B, 2*base, H/2, W/2]
        encoded = self.level3(skip2)  # [B, 4*base, H/4, W/4]
...
class SharedCausalDenoiser(nn.Module):
    ...
    def forward(self, history: Tensor, stage_id: Tensor) -> Tuple[Tensor, Tensor]:
        ...
        denoised = history[:, -1] + residual  # residual denoising formulation
...
class FusionHead(nn.Module):
    ...
    def __init__(self, image_channels: int = 3, latent_dim: int = 128, sensor_dim: int = 10, root_classes: int = 3) -> None:
        ...
        self.root_cause_logits = nn.Linear(128, root_classes)  # powder / scan / unknown
...
if __name__ == "__main__":
    # Shape-only smoke test. This does not read AMMT TIFF files.
