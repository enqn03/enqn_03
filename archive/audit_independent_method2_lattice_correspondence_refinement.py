...
def propagate_provisional_2d_labels(points: np.ndarray, responses: np.ndarray, edges: list[dict[str, Any]]) -> tuple[dict[int, tuple[int, int]], dict[str, Any]]:
    ...
    root = int(np.argmax(np.asarray(responses, dtype=np.float64)))
...
def refine_correspondence(points: np.ndarray, responses: np.ndarray) -> tuple[list[dict[str, Any]], dict[str, Any], dict[str, Any], list[dict[str, Any]], np.ndarray, np.ndarray]:
    ...
    """
    edges, graph_metrics, _, degree = nearest_neighbor_graph(points)
    labels, propagation_metrics = propagate_provisional_2d_labels(points, responses, edges)
...
def main() -> None:
    ...
    summary = {
        ...
        "refined_2d_correspondence": {
            "method": "local neighbor graph plus projective reassignment under a 50x50 image-lattice candidate grid",
        ...
        "storage_policy": "writes two compact CSVs, one JSON summary, and exactly three deterministic QC overlays; no dense crop, rectified image, mask, heatmap, target, or model output is persisted",
        "outputs": {
