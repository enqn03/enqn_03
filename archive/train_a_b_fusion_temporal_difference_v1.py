...
class ABFusionCausalTemporalDifferenceCandidateNet(nn.Module):
    """A+B fusion model with paired temporal-difference branches.

    Processes A and B independently through paired encoders, then concatenates
    the fused features and decodes them to a single score map.
    """
    ...
    def __init__(self, input_channels: int = 6, base_channels: int = 32) -> None:
        ...
        # Decoder receives concatenated A and B features (2 * base_channels)
    ...
    def forward(self, history_a: Tensor, history_b: Tensor) -> Tensor:
        ...
        fused_ab = torch.cat([fused_a, fused_b], dim=1)
        return torch.sigmoid(self.decoder(fused_ab))
...
def validate_config(config: dict[str, Any]) -> None:
    ...
    if model["name"] != "a_b_fusion_causal_temporal_difference_candidate_net":
        raise ValueError("Unexpected model.name for fusion experiment.")
    if model["temporal_fusion"] != "paired_encoders_with_feature_concatenation":
...
def dry_run(model: nn.Module, dataset: Any, loss_fn: SupportMaskedSmoothL1Loss, device: torch.device, sample_index: int) -> None:
    ...
    print(
        json.dumps(
            {
                "audit_type": "A+B Fusion temporal-difference controlled experiment dry run",
                "architecture": "paired_encoders_with_feature_concatenation",
    ...
    print("A+B Fusion dry run complete.")
...
def run_fusion_epoch(
    ...
    # Need to redefine run_epoch since inputs changed to two histories
    ...
    for batch_idx, batch in enumerate(loader):
        ...
        if batch_supported_pixels > 0:
            ...
            # Add number of samples in this batch that had support
            ...
            # Skip optimizer step when batch has no support
            pass
...
def main() -> None:
    ...
    model = ABFusionCausalTemporalDifferenceCandidateNet(
    ...
    write_json(
        ...
        {
            "purpose": "A+B Fusion Temporal-difference training history",
    ...
    write_json(
        ...
        {
            "purpose": "A+B Fusion causal test metric on sparse XCT-supported pixels",
    ...
    write_json(
        ...
        {
            "purpose": "A+B Fusion compact raw-camera coordinate candidates",
            "stage": "fusion",
            "response_direction": "unresolved",
        ...
        json.dumps(
            {
                ...
                "architecture": "a_b_fusion_temporal_difference",
                "device": str(device),
                "best_epoch": best_epoch,
                "best_validation_supported_pixel_mean_smooth_l1": best_validation_loss,
                "test": asdict(test_metrics),
    ...
    print("A+B Fusion Temporal-difference controlled experiment complete.")
