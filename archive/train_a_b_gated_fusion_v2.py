...
class GatedFusion(nn.Module):
    """Spatio-channel attention gate to selectively fuse A and B features."""
    ...
    def forward(self, feat_a: Tensor, feat_b: Tensor) -> Tensor:
        ...
        # Compute soft attention gate for the combined features
        gates = torch.sigmoid(self.attention(concat))
        # Multiply concatenated features by their respective learned gates
        return concat * gates


class ABGatedFusionCausalTemporalDifferenceCandidateNet(nn.Module):
    """A+B fusion model with paired temporal-difference branches and Gated CNN.

    Processes A and B independently through paired encoders, then selectively
    weights their concatenated features using a learned spatial-channel gate,
    ...
    def __init__(self, input_channels: int = 6, base_channels: int = 32) -> None:
        ...
        # Decoder receives gated concatenated A and B features (2 * base_channels)
    ...
    def forward(self, history_a: Tensor, history_b: Tensor) -> Tensor:
        ...
        # Apply the learned Gated CNN Fusion instead of raw concatenation
...
def validate_config(config: dict[str, Any]) -> None:
    ...
    if model["name"] != "a_b_gated_fusion_causal_temporal_difference_candidate_net":
...
def main() -> None:
    ...
    model = ABGatedFusionCausalTemporalDifferenceCandidateNet(
