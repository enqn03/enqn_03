...
class AMMTTiffMemmapReader:
    ...
    def _read_header(self) -> HyperstackInfo:
        # TiffFile.series gives the logical hyperstack shape without decoding
        # all data. ImageJ metadata preserves Z (manufacturing layer) and T
        # (LED direction) even if TIFF has only one physical page.
        with tifffile.TiffFile(self.path) as tif:
            ...
            # Tifffile normally exposes Y and X. For AMMT, metadata gives
            # robust dimensions even if it exposes a generic leading axis Q.
    ...
    def _ensure_memmap(self) -> np.memmap:
        if self._data is None:
            try:
                # mode='r': prevents accidental modification of original TIFF
    ...
    def close(self) -> None:
        # ndarray/memmap reference cleanup; no source file modification
        self._data = None

    def __getstate__(self):
        # macOS DataLoader workers use spawn. Do not pickle an open memmap.
    ...
    def read_frame(self, z: int, led: int, channel: int = 1) -> np.ndarray:
        ...
        if "Z" in axes and "T" in axes:
            ...
            # ImageJ XYCZT flatten order: C varies fastest, then Z, then T.
...
class AMMTLayerSequenceDataset(Dataset):
    ...
    def __getitem__(self, index: int) -> dict[str, torch.Tensor | int]:
        ...
        for z in range(z_start, z_start + self.sequence_length):
            ...
            timesteps.append(torch.stack(led_channels, dim=0))  # [C, H, W]

        return {
            "x": torch.stack(timesteps, dim=0),  # [T, C, H, W]
            "z_start": z_start,
