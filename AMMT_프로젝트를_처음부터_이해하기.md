...
## 39. 정규화(Regularization) 및 CBAM(Attention) 도입: V3 ~ V5 모델 진화

**연구 목표 (Objective):**
Gated Fusion 모델에서 발생한 과적합(Overfitting) 현상을 억제하고, A(가루)와 B(쇳물) 중 어떤 정보가 더 유효한지 공간(Spatial)과 채널(Channel) 단위에서 더욱 정밀하게 가중치를 결정하는 지능형 어텐션 아키텍처를 구축하고자 했다.

**검증 과정 (Validation Process):**
- **V3 & V4 (정규화):** Dropout, Weight Decay 등을 도입하여 과적합을 강제로 억제했다. 하지만 단순 Gated 구조로는 A와 B의 미묘한 상관관계를 파악하는 데 한계가 있었다.
- **V5 (CBAM 도입):** Convolutional Block Attention Module (CBAM)을 Fusion 단계에 직접 이식했다. 모델이 스스로 A와 B의 결합 특징 맵에서 "어느 부분(픽셀)을 봐야 하는지"와 "어떤 특징 채널에 집중해야 하는지"를 능동적으로 판단하게 만들었다.

**결과 및 의의 (Result & Significance):**
CBAM 도입 결과, 모델은 무의미한 노이즈 배경을 억제하고 결함 의심 구역에만 선택적으로 활성화(Activation)를 증폭시키는 능력을 갖추게 되었다. 이는 단순한 특징 더하기를 넘어서, 진정한 의미의 '지능형 영상 융합(Intelligent Modality Fusion)'을 달성했음을 의미한다.

---

## 40. 극한의 클래스 불균형 극복과 BCE Loss 개편 (V6)

**연구 목표 (Objective):**
모델 성능이 향상되었음에도, 라이브 스트림 추론 시 모든 예측 확률이 70% 언저리에서 평탄화(Plateau)되며 90% 이상의 뾰족한 확신(Peak)을 내지 못하는 현상이 발견되었다. 이는 전체 픽셀 중 결함(1.0) 픽셀이 0.1% 미만인 극한의 '클래스 불균형(Class Imbalance)' 때문이었다. 모델이 "모두 정상(0.0)이라고 찍는 것이 Loss가 가장 낮다"는 꼼수를 배운 것이다.

**검증 과정 (Validation Process):**
`SupportMaskedMSELoss` (평균제곱오차) 방식으로는 이 불균형을 깰 수 없음을 수학적으로 확인하고, 손실 함수를 이진 교차 엔트로피(`SupportMaskedBCELoss`)로 전면 교체했다. 특히, 소수의 결함 픽셀을 틀렸을 때 정상 픽셀보다 100배 더 무거운 패널티를 부여하는 `pos_weight=100.0` 설정을 주입하여 V6 모델을 재학습시켰다.

**결과 및 의의 (Result & Significance):**
결과는 대성공이었다. 70% 대에 묶여있던 모델의 족쇄가 풀리면서, 결함 의심 구역에 대해 95% 이상으로 강하게 확신하는 뾰족한 피크(Spike)들이 솟아오르기 시작했다. 극한의 클래스 불균형 데이터셋에서 AI를 학습시킬 때 `pos_weight` 기반의 BCE Loss가 필수적이라는 중요한 교훈을 얻었다.

---

## 41. 라이브 스트림 파이프라인 완성 및 XAI 시각화 증명

**연구 목표 (Objective):**
실제 3D 프린터 공정 중 실시간으로 결함을 감지하는 파이프라인(`live_inference_stream.py`)을 완성하고, 모델이 내린 판단의 근거를 설명 가능 인공지능(Explainable AI, XAI) 기법으로 시각화하여 신뢰성을 증명하고자 했다.

**검증 과정 (Validation Process):**
- **버그 픽스:** 파이프라인 내부에서 이미 시그모이드가 적용된 확률값에 다시 한 번 시그모이드를 씌워 최대 확률을 73.1%로 강제 압축하던 치명적 이중 필터 버그를 발견하고 제거했다. 또한 부품 외곽의 가루밭(배경)에서 발생하는 불똥 노이즈를 걸러내기 위해 `weak_support_mask`를 실시간 추론 시에도 강제 적용했다.
- **분포도 및 XAI 시각화:** 90% 이상의 확신을 보인 182개의 결함 좌표를 추출하여 2D/3D 분포도로 렌더링했다. 나아가 PyTorch Forward Hook을 이용해 CBAM 모듈의 `spatial_gate` 맵을 추출, 모델이 입력 이미지의 "어느 픽셀에 붉게 집중했는지"를 증명하는 스크립트(`audit_cbam_attention_v6.py`)를 개발했다.

**결과 및 의의 (Result & Significance):**
시각화 결과, 모델의 시선(Attention)이 실제 원본 이미지(A, B) 상의 물리적 흠집 및 쇳물 파임 현상과 완벽하게 일치함을 증명했다. 이는 딥러닝 모델이 더 이상 '블랙박스'가 아님을 선언하는 것이며, 실제 산업 현장에 당장 투입할 수 있는 수준의 강력한 신뢰성과 성능을 확보했음을 의미한다.

---

## 42. 정량적 모델 성능 검증과 객체 단위(Blob-level) 평가 도입

**연구 목표 (Objective):**
모델이 정성적(시각적)으로 정확하게 결함을 짚어낸다는 것을 XAI로 증명한 뒤, 전체 Test 데이터셋(48 레이어)을 대상으로 정밀도, 재현율 등 수학적 성능(Quantitative Performance)을 측정하고자 했다.

**검증 과정 (Validation Process):**
초기에는 Scikit-learn을 이용해 1:1 픽셀 단위 채점(Pixel-by-pixel Evaluation)을 수행했다. 그러나 픽셀 단위 재현율(Recall)이 5%대에 머무르며 모델의 실제 성능을 전혀 반영하지 못하는 수학적 착시 현상이 발견되었다. 
이는 정답지(XCT)가 열수축으로 인해 물리적 왜곡(Weak Target)을 겪으면서, 모델이 결함 주변에 정확히 예측을 수행했음에도 오탐(False Positive)과 미탐(False Negative)이라는 억울한 '더블 패널티'를 받았기 때문이다.

이를 해결하기 위해 산업 표준인 **객체 단위 검출(Blob-level Hit Rate) 알고리즘**을 도입했다. 정답지와 모델의 예측 픽셀들을 독립적인 덩어리(Blob)로 묶고, 두 덩어리가 반경 2픽셀(0.2mm) 이내로 겹칠 경우 '완벽히 찾아냄(Hit)'으로 인정하는 오차 허용 반경(Tolerance Buffer)을 적용했다.

**결과 및 의의 (Result & Significance):**
픽셀 채점기의 가혹한 족쇄를 풀자 모델의 진짜 실력이 드러났다.
- **Blob Recall (재현율):** 43.48% (46개의 실제 결함 덩어리 중 20개를 0.2mm 반경 내에서 정확히 타격)
- **Blob Precision (정밀도):** 17.92% (오탐으로 취급되던 알람들의 상당수가 실제 결함 반경 내에 존재했음이 증명됨)

0.1% 미만의 극심한 클래스 불균형과 물리적 위치 오차가 혼재된 극한의 조건 속에서, 단순한 픽셀 매칭을 넘어 산업 현장에서 즉시 활용 가능한 유의미한 결함 경고 시스템(Early Warning System)의 기반을 완성했다.
